...
filter :client
  ...
  scope :performa_invoice  do |invoices|
    invoices.where(:status => Invoice::STATUS_PI)
  end

  scope :purchase_order do |invoices|
    invoices.where(:status => Invoice::STATUS_PO)
  end
  
  scope :sales_order do |invoices|
    invoices.where(:status => Invoice::STATUS_SO)
  end
  ...
  action_item :only => :show do
    link_to "PRINT PI", generate_pdf_admin_invoice_path(resource)
  end
  ...
  action_item :only => :show do
    link_to "for PO", send_invoice_admin_invoice_path(resource)
  end
  ...
  member_action :dispatch_invoice, :method => :post do
    ...
    @invoice.status = Invoice::STATUS_PO
    @invoice.save
    
    redirect_to admin_invoice_path(@invoice), :notice => "Invoice sent to Client for PO"
  end
